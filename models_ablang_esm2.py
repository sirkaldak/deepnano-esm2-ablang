...
# AbLang encoder (你已经创建了 models/ablang_encoder.py)
from models.ablang_encoder import AbLangHeavyEncoder


def _masked_pool_min_mean_max(x: torch.Tensor, mask: torch.Tensor):
    """
    x: (B, L, D)
    mask: (B, L) bool, True=valid
    returns: (mean, min, max) each (B, D)
    """
    mask_f = mask.unsqueeze(-1).float()  # (B, L, 1)

    mean = (x * mask_f).sum(dim=1) / mask_f.sum(dim=1).clamp(min=1.0)

    x_max = x.masked_fill(~mask.unsqueeze(-1), float("-inf"))
    maxv = x_max.max(dim=1).values

    x_min = x.masked_fill(~mask.unsqueeze(-1), float("inf"))
    minv = x_min.min(dim=1).values
...
class baseline(nn.Module):
        ...
        if finetune == 0:  # freeze ESM2
            ...
            for name, param in self.pretrained_model.named_parameters():
                ...
                if 'esm2_t48_15B_UR50D' in pretrained_model and 'encoder.layer.47.' not in name:
                    param.requires_grad = False
        elif finetune == 2:  # finetune all
            for param in self.pretrained_model.parameters():
                param.requires_grad = True

        # Nanobody uses AbLang
        try:
            self.nb_encoder = AbLangHeavyEncoder(freeze=True)
        except TypeError:
            self.nb_encoder = AbLangHeavyEncoder()

        self.nb_proj = nn.LazyLinear(hidden_size)
    ...
    def forward(self, seq1, seq2, device):
        # ===== Nanobody (AbLang -> mean pooling) =====
        nb_res, nb_mask = self.nb_encoder(seq1)      # (B, L, 768), (B, L)
        nb_res = nb_res.to(device)
        nb_mask = nb_mask.to(device)
        nb_res = self.nb_proj(nb_res)               # (B, L, hidden)
        feature_seq1_ave, _, _ = _masked_pool_min_mean_max(nb_res, nb_mask)

        # ===== Antigen (ESM2 -> mean pooling) =====
        tokenizer2 = self.tokenizer(
            seq2, truncation=True, padding=True, max_length=800, add_special_tokens=False
        )
...
class DeepNano_seq(nn.Module):
    ...
    def forward(self, seq1, seq2, device):
        # ===== Nanobody (AbLang -> min/mean/max pooling) =====
        nb_res, nb_mask = self.nb_encoder(seq1)
        nb_res = nb_res.to(device)
        nb_mask = nb_mask.to(device)
        nb_res = self.nb_proj(nb_res)
        feature_seq1_ave, feature_seq1_min, feature_seq1_max = _masked_pool_min_mean_max(nb_res, nb_mask)

        # ===== Antigen (ESM2 -> min/mean/max pooling) =====
        tokenizer2 = self.tokenizer(
            seq2, truncation=True, padding=True, max_length=800, add_special_tokens=False
        )
        input_ids2 = torch.tensor(tokenizer2['input_ids']).to(device)
...
class DeepNano_site(nn.Module):
    def __init__(self, pretrained_model=r'./models/esm2_t6_8M_UR50D', hidden_size=320, finetune=1):
        ...
        # Nanobody uses AbLang (only need mean)
        try:
            self.nb_encoder = AbLangHeavyEncoder(freeze=True)
        except TypeError:
            self.nb_encoder = AbLangHeavyEncoder()
    ...
    def forward(self, seq1, seq2, device):
        ...
        attention_mask2 = torch.tensor(tokenizer2['attention_mask']).to(device)
        temp_output2 = self.pretrained_model(input_ids=input_ids2, attention_mask=attention_mask2)
        feature_seq2_ave = temp_output2.last_hidden_state  # (B, L2, hidden)
...
class DeepNano(nn.Module):
    def __init__(self, pretrained_model=r'./models/esm2_t6_8M_UR50D', hidden_size=320, d_prompt=8, finetune=1,
        ...
        # Antigen_BSite predictor (use our modified DeepNano_site)
    ...
    def forward(self, seq1, seq2, device):
        ...
        # add prompt to antigen pooled vectors
